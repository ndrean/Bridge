...
const Args = struct {
    ...
    pub fn parseArgs(allocator: std.mem.Allocator) !Args {
        ...
        var slot_name: []const u8 = "bridge_demo_slot"; // default
        var publication_name: []const u8 = "bridge_demo_pub"; // default
                ...
                if (args.next()) |value| {
                    ...
                    if (std.ascii.eqlIgnoreCase(value, "all")) {
                        ...
                        while (iter.next()) |table_name| {
                            const trimmed = std.mem.trim(u8, table_name, &std.ascii.whitespace);
                            if (trimmed.len > 0) {
                                try table_list.append(allocator, trimmed);
                            }
                        }
...
pub fn main() !void {
    ...
    var http_srv = try http_server.Server.init(allocator, parsed_args.http_port, &should_stop, &metrics, null);
    const http_thread = try std.Thread.spawn(.{}, http_server.Server.run, .{&http_srv});
    ...
    const monitor_thread = try std.Thread.spawn(.{}, wal_monitor.monitorWalLag, .{ &metrics, monitor_config, &should_stop, allocator });
    ...
    log.info("5. Streaming CDC events from PostgreSQL to NATS...\n", .{});
    log.info("   Subject pattern: {s}\n", .{lower_subject});
    log.info("   Generate events with: cd consumer && mix run -e 'Producer.run_test(500)'\n", .{});
    ...
    log.info("To monitor the NATS stream:", .{});
    log.info("  nats stream info CDC_BRIDGE", .{});
    log.info("  nats sub 'cdc.>'", .{});
    log.info("\nTo generate more events:", .{});
    log.info("  cd consumer && mix run -e 'Producer.run_test(N)'", .{});
}
